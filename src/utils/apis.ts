...
export async function getNFTsForWallet(
  ...
  const settings = {
    ...
    network: ALCHEMY_CHAIN_SLUGS[chain as keyof typeof ALCHEMY_CHAIN_SLUGS] as Network,
  }
  const chainId = CHAIND_TO_CHAIN_IDS[chain as keyof typeof CHAIND_TO_CHAIN_IDS]
  ...
  const nfts = sortAndFilterNfts(nftsForOwner.ownedNfts, chainId, walletAddress)
...
export async function getCryptoPunksforWallet(
  ...
  if (data.result) {
    return data.result.map((punk) => convertCryptoPunkToNFT(punk, chainId, walletAddress))
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function sortAndFilterNfts(nftData: any[], chainId: number, walletAddress: string): any[] {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function convertCryptoPunkToNFT(punk: any, chainId: number, walletAddress: string): NFT {
