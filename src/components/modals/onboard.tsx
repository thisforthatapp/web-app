...
const Onboard: React.FC<Props> = ({ closeModal }) => {
  ...
  const addWallet = () => {
    if (newWallet) {
      ...
      // Here you would typically fetch NFTs for this wallet
      // For this example, we'll just add some dummy NFTs
      setNfts([
        ...nfts,
        { id: nfts.length + 1, name: "NFT 1", willSwap: false },
        { id: nfts.length + 2, name: "NFT 2", willSwap: false },
      ]);
      setNewWallet("");
    }
  };

  const toggleNftSwap = (id) => {
    setNfts(
      nfts.map((nft) =>
        nft.id === id ? { ...nft, willSwap: !nft.willSwap } : nft
      )
    );
  };
  */
  ...
  return (
    ...
    >
      {completeScreen && (
        <div className="flex flex-col items-center py-6 px-12 items-center justify-center text-center">
          <div className="text-8xl">🎉</div>
          <div className="my-4">
            Congratulations, <b>{username}</b>.
          </div>
          <div>You've finished setting up your TFT account!</div>
          <div className="my-2">
            It's time to go swapping. Share your profile and check out what NFTS
            people are offering. Happy swapping!
          </div>
          <div className="font-semibold cursor-pointer mt-2">
            www.thisforthat.app/{username}
          </div>
          <button className="mt-8 bg-indigo-500 text-white w-[350px] h-[45px] rounded font-semibold text-lg">
            Let's Go!
          </button>
        </div>
      )}
      {!completeScreen && (
        <>
          ...
          >
            ...
            <div className="flex items-center justify-end mt-auto px-6 py-4">
              <button
          ...
          {step === 2 && (
            <>
              ...
              <div className="flex items-center justify-end mt-auto px-6 py-4 gap-x-4">
                <button
                  className={`bg-gray-200 text-black font-semibold py-2 px-4 rounded focus:outline-none focus:shadow-outline`}
                >
                  Skip
                </button>
                <button
                  className={`bg-gray-900 hover:bg-gray-700 text-white font-semibold py-2 px-4 rounded focus:outline-none focus:shadow-outline`}
